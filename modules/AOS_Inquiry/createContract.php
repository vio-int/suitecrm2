 ...
 */
    ...
    $sql = "SELECT * FROM aos_line_item_groups WHERE parent_type = 'AOS_Inquiry' AND parent_id = '".$quote->id."' AND deleted = 0";
    ...
    //Setting Line Items
    $sql = "SELECT * FROM aos_products_quotes WHERE parent_type = 'AOS_Inquiry' AND parent_id = '".$quote->id."' AND deleted = 0";
    $result = $this->bean->db->query($sql);
    while ($row = $this->bean->db->fetchByAssoc($result)) {
        $row['id'] = '';
        $row['parent_id'] = $contract->id;
	...
	$key = Relationship::retrieve_by_modules('AOS_Inquiry', 'AOS_Contracts', $GLOBALS['db']);
	if (!empty($key)) {
